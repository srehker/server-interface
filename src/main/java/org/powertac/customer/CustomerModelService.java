 ...
 * ConfigurableInstances. Each gets supplied a set of service interfaces. In
 * each timeslot, each model has its step() method called. When tariffs are
 * published, each has its evaluateTariffs() method called. At the end of a boot
 * session, each has its bootstrap state saved to the boot record.
 * 
 * @author John Collins
 */
@Service
public class CustomerModelService extends TimeslotPhaseProcessor implements
		InitializationService, BootstrapState, NewTariffListener,
		CustomerServiceAccessor {
	static private Logger log = Logger.getLogger(CustomerModelService.class
			.getName());
	...
	public String initialize(Competition competition,
		...
		ServiceLoader<AbstractCustomer> loader = ServiceLoader
				.load(AbstractCustomer.class);
		...
		while (modelIterator.hasNext()) {
			...
			for (Object modelObj : serverConfig.configureInstances(modelEx
					.getClass())) {
	 ...
	 * 
	 * @see
	 * org.powertac.common.interfaces.TimeslotPhaseProcessor#activate(org.joda
	 * .time.Instant, int)
	 */
	@Override
